...
NUM_LEARN_EPISODES = 10000
NUM_PLAY_EPISODES = 600
###########################################
...
class Agent():
    ...
    def select_action(self, state):
        ...
        probs = self.sess.run(self.action_probs, feed_dict = {self.states : state.reshape([1, 4])})
...
def main():
    if os.path.exists(MODEL_DIR) and not REPLACE_MODEL:
        play()
    else:
        learn()
